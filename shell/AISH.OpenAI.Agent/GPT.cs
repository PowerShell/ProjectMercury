...
public class GPT
{
    ...
    private async Task AskForKeyAsync(IHost host, CancellationToken cancellationToken)
    {
        ...
        if (Utils.AISHEndpoint.Equals(Endpoint, StringComparison.OrdinalIgnoreCase))
        {
            host.MarkupLine(" [grey]> The model uses the default AISH endpoint.[/]")
