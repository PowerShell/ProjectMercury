    ...
    public partial class PSConsoleReadLine
    {
        ...
        private void SetKeyHandlerInternal(string[] keys, Action<ConsoleKeyInfo?, object> handler, string briefDescription, string longDescription)
        {
            foreach (var key in keys)
            {
                ...
                if (chord.Length == 1)
                {
                    _dispatchTable[firstKey] = MakeKeyHandler(handler, briefDescription, longDescription);
                }
                else
                {
                    ...
                    secondDispatchTable[PSKeyInfo.FromConsoleKeyInfo(chord[1])] = MakeKeyHandler(handler, briefDescription, longDescription);
                }
            }
        }
